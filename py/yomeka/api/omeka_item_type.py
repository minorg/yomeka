...
class OmekaItemType(object):
    class Builder(object):
        ...
        def build(self):
            return OmekaItemType(id=self.__id, name=self.__name, url=self.__url)

        @classmethod
        def from_template(cls, template):
            '''
            :type template: yomeka.api.omeka_item_type.OmekaItemType
        ...
        def update(self, omeka_item_type):
            if isinstance(omeka_item_type, OmekaItemType):
                self.set_id(omeka_item_type.id)
                self.set_name(omeka_item_type.name)
                self.set_url(omeka_item_type.url)
            elif isinstance(omeka_item_type, dict):
                for key, value in omeka_item_type.items():
                    getattr(self, 'set_' + key)(value)
            else:
                raise TypeError(omeka_item_type)
            return self
    ...
    def write(self, oprot):
        ...
        oprot.write_struct_begin('OmekaItemType')
